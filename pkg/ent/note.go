...
type Note struct {
	...
	// BinderID holds the value of the "binder_id" field.
	BinderID uint `json:"binder_id,omitempty"`
...
func (*Note) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		switch columns[i] {
		case note.FieldID, note.FieldBinderID:
			values[i] = new(sql.NullInt64)
		case note.FieldPublisher, note.FieldComment, note.FieldContent:
	...
	for i := range columns {
		...
		case note.FieldBinderID:
			if value, ok := values[i].(*sql.NullInt64); !ok {
				return fmt.Errorf("unexpected type %T for field binder_id", values[i])
			} else if value.Valid {
				n.BinderID = uint(value.Int64)
			}
		case note.FieldPublisher:
			if value, ok := values[i].(*sql.NullString); !ok {
				return fmt.Errorf("unexpected type %T for field publisher", values[i])
			} else if value.Valid {
				n.Publisher = value.String
			}
...
func (n *Note) String() string {
	...
	builder.WriteString("binder_id=")
	builder.WriteString(fmt.Sprintf("%v", n.BinderID))
	builder.WriteString(", ")
	builder.WriteString("publisher=")
	builder.WriteString(n.Publisher)
	builder.WriteString(", ")
